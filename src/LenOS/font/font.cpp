...
Graphic::RectType Font::window(Graphic::Point(0, 0), Screen::width(), Screen::high());

/**********************************获取字符的字体信息开始******************************/
/*
	此函数从ascii8x16中获取一字符的信息
*/
const Font::FontInfo& Font::ascii_8x16_char_info(const char c)
{
	static FontInfo fi;
	//如果不在可视范围内，返回0
	if(c < 0x20 || c > 126)
...
#ifdef __ASCII_10x19
/*
	此函数从ascii_10x19中获取一字符的信息
*/
const Font::FontInfo&  Font::ascii_10x19_char_info(const char c)
{
	static FontInfo fi;
	//如果不在可视范围内，返回0
	if(c < 0x20 || c > 126)
...
const Font::FontInfo& Font::ascii_YAHEI_char_info(const char c)
{
	static FontInfo fi;
	//如果不在可视范围内，返回0
	if(c < 0x20 || c > 126)
...
const Font::FontInfo& Font::char_info(const char c)
{
	switch(fontFormat.fontType)
	{
		case Ascii_8x16:
...
U32 Font::string_width(const char *str, FontType fontType)
{
	char c;
	U32 width = 0;
	do
...
//根据字体的类型得到一个字符的宽度
U32 Font::char_width(const char c, FontType fontType)
{
	return font_info(c, fontType).fontWidth + fontFormat.rowSpacing;
}

//根据
U32 Font::char_high(const char c, FontType fontType)
{
	return font_info(c, fontType).fontHigh + fontFormat.lineSpacing;
}


const Font::FontInfo& Font::font_info(const char c, const FontType& type)
{
	switch(type)
	{
		case Ascii_8x16:
...
Graphic::RectType Font::set_window(Graphic::RectType r)
{
	Graphic::RectType old = window;
	r.xSize = r.xSize * font_width();
	r.ySize = r.ySize * font_high();
	window = r;
	return old;
}

Graphic::RectType Font::set_full_window()
{
	Graphic::RectType old = window;
	window = Graphic::RectType(Graphic::Point(0, 0), Screen::width(), Screen::high());
	return old;
}
